...
class _DoctorReportsPageState extends State<DoctorReportsPage> {
  final int _selectedIndex = 3; // Currently selected "Reports"
  String? doctorId;
  int monthlyAppointments = 0; // Monthly appointments count
  int yearlyPatients = 0; // Yearly patients count
  ...
  Future<void> _fetchAppointmentsAndPatients() async {
    ...
    try {
      ...
      int totalAppointmentsCurrentMonth = 0;
      ...
      for (var userDoc in usersSnapshot.docs) {
        ...
        for (var appointmentDoc in appointmentsSnapshot.docs) {
          ...
          if (appointmentDate is DateTime && appointmentDate.month == currentMonth) {
            totalAppointmentsCurrentMonth++;

            if (appointmentDate.isAfter(startOfYear.subtract(Duration(days: 1)))) {
              uniquePatients.add(appointmentData['userId']);
            }
          }
        }
      }
      ...
      List<int> fetchedData = await fetchForEveryMonthApptsCounts();
      setState(() {
        monthlyAppointmentsCount = fetchedData;
      });
    } catch (e) {
            ...
            return BarChartGroupData(
              ...
              barRods: [
                BarChartRodData(
                  ...
                  color: _getMonthColor(index), // **使用不同颜色**
                  width: 16,
...
Color _getMonthColor(int index) {
  List<Color> colors = [
      ...
      Colors.orangeAccent,  // March
      Colors.amberAccent,   // April
      Colors.limeAccent,    // May
      Colors.yellowAccent,  // June
      Colors.greenAccent,   // July
      Colors.cyanAccent,    // August
      Colors.tealAccent,    // September
      Colors.blueAccent,    // October
      Colors.indigoAccent,  // November
      Colors.deepPurpleAccent, // December
  ];
